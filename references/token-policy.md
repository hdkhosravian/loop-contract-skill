...
Quality tracks the same curve. Retrieval accuracy degrades well before the window is full, so a loop
that runs to 90% context is producing its worst work in the turns that matter most.
...
Typical audits are 60–70% deterministic. That is 60–70% of items resolved for a rounding error, with
