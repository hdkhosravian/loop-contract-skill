...
<dir>/
  ...
  shards.jsonl       # GENERATED by index_corpus.py. the corpus manifest = the coverage contract.
  coverage.jsonl     # APPEND-ONLY. one disposition per shard: harvested | no-requirements | unreadable.
  ...
  HARVEST.md         # generated by verify_harvest.py. grounding + coverage evidence.
...
scripts/index_corpus.py    # deterministic corpus enumeration -> shards.jsonl
scripts/verify_harvest.py  # deterministic grounding + coverage gate (P1)
scripts/fold.py            # deterministic fold + completion gate (P5)
```

`shards.jsonl` is generated, never hand-edited: editing it edits the definition of "the whole corpus",
which is the one thing in the spine that must not be negotiable.

Ledger row schema — every row in `ledger.jsonl`, whichever phase writes it:

```json
{"id":"<str>","src":"<path:line>","shard":"<shard id>","section":"<heading path>",
 "kind":"gate|ac|task|normative|constant|gap|invariant|threshold|<job-specific>",
 "claim":"<verbatim text — never paraphrased>","check":"deterministic|judgement|unclassified",
 "status":"OPEN","confidence":"high|med|low"}
```

`src` and `claim` are load-bearing, not decoration: `verify_harvest.py` re-reads the file and fails
the phase unless the claim is really there. `shard` ties the row back to the manifest so a source that
was never read is a non-zero exit rather than a gap nobody notices.

`check` steers the P2→P3 cascade — harvesters set it to `unclassified`; P2 resolves each to
`deterministic` or `judgement`. `confidence:"low"` marks a row the harvester was unsure was a
requirement at all; P2 settles those first, since a wrong one costs a single grep to discard.
`fold.py` reads neither `check` nor `status`; it joins on `id` against `verdicts.jsonl`. (A harvester
may add `doc_says_done` on a checked task box — surfaced in the report's "marked done but not
verified" section.)
...
### P1 — Harvest (the one expensive-but-necessary phase)

Full protocol in `references/harvest-protocol.md`. Four steps, and the model only owns the middle two:

```bash
python scripts/index_corpus.py <corpus paths> --ext <exts> -o <dir>/shards.jsonl   # H0
```
Read the SKIPPED list it prints — that is the only place a lost source can still hide.

H1: dispatch harvesters, 10–20 shards each, under the spawn contract in the protocol. Verbatim claims,
`path:line` citations, a coverage record for **every** shard including the empty ones. Recall over
precision: an unsure row is included with `confidence:"low"`, because a false row dies in P2 for the
price of one grep while a missed row is never audited at all.

```bash
python scripts/verify_harvest.py --ledger <dir>/ledger.jsonl --shards <dir>/shards.jsonl \
       --coverage <dir>/coverage.jsonl --out <dir>/HARVEST.md                      # H2
```

H3: adversarial recall sweep over the zero-row shards plus <20>% of the rest, max two rounds. Record
the find-rate — it is the recall estimate, and it belongs in the report.

**Gate P1:** `verify_harvest.py` exits 0 **and** the last sweep round added < 2% new rows. Print all
five figures — shards · rows · ungrounded · unharvested · sweep find-rate. A row count on its own is a
count, not coverage. Never widen `--window` or hand-edit `shards.jsonl` to make this gate pass; both
