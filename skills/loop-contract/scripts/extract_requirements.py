#!/usr/bin/env python3
"""
extract_requirements.py — a RECALL NET over markdown. Not the extractor.
...
READ THIS BEFORE RELYING ON IT. This script finds only the conventions its patterns
anticipate — "Gate:", "AC-3:", "- [ ]", MUST/SHALL, ID-shaped table rows. A project
that states its requirements as ordinary prose (ADRs, design docs, most specs) will
produce almost nothing useful here, and the ID pattern over-matches badly on
reference-heavy documents. Deciding what a document asks for is comprehension and
belongs to the model; this script is the second pass that catches what the reader
missed. Enumerate scope by reading the source, then run this and investigate only
the hits you did not already have. Never present its output as the task list, and
never treat its count as the authority.
...
PATTERNS = [
    # "Gate: every write goes through the audit log"
    ...
    # "| MAX_RETRIES | 5 |"  or  "MAX_RETRIES = 5"  or  "`X`: 0.15"
